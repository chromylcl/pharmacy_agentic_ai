...
const CartDrawer = ({ isOpen, onClose }) => {
  const { cart, setCart, setIsBillingOpen } = usePharmacy();

  const grandTotal = cart.reduce((sum, item) => sum + (parseFloat(item.price) * parseInt(item.quantity || 1)), 0);
            ...
            cart.map((item, idx) => (
              <div key={idx} className="bg-white/60 backdrop-blur-md border border-white/80 p-4 rounded-3xl shadow-sm flex items-center justify-between group hover:bg-white/80 transition-colors">
                <div>
                  ...
                  <p className="text-xs font-semibold text-slate-500 mt-0.5 uppercase tracking-wider">Qty: {item.quantity || 1}</p>
                </div>
                <div className="flex items-center gap-4">
                  <span className="font-extrabold text-slate-800">${(parseFloat(item.price) * (item.quantity || 1)).toFixed(2)}</span>
                  <button
